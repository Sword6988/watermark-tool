...
def plaintext_dir() -> str:
    ...
    with _state_lock:
        if _plain_dir is None or not os.path.isdir(_plain_dir):
            _plain_dir = tempfile.mkdtemp(prefix="wm-dlp-")
        return _plain_dir
...
def _build_provider() -> Optional[DecryptProvider]:
    """按环境变量装配解密器（**不在代码里硬编码任何第三方工具**）。

    * ``WM_DLP_DECRYPT_CMD`` —— 命令模板，形如
      ``"C:\\Tools\\dec.exe" "{src}"``；**不填就等于没有解密器**。
    * ``WM_DLP_ENABLE``      —— ``0/off/false`` 可一键关停（保留命令配置）。
    * ``WM_DLP_TIMEOUT``     —— 单文件解密超时秒数（默认 30）。
    ...
    if flag in ("0", "false", "no", "off"):
        return None
    try:
        timeout = float((os.environ.get("WM_DLP_TIMEOUT") or "").strip() or DEFAULT_TIMEOUT)
    except ValueError:
